"""Config flow for Statistics Calendar."""
...
from .const import DOMAIN, SUPPORTED_WINDOW_MODES, WINDOW_MODE, WINDOW_ROLLING
from .sensor import (
...
async def get_state_characteristics(handler: SchemaCommonFlowHandler) -> vol.Schema:
    ...
    options = list(STATS_BINARY_SUPPORT) if is_binary else list(STATS_NUMERIC_SUPPORT)
...
async def validate_options(
    ...
    window_mode = user_input.get(WINDOW_MODE, handler.options.get(WINDOW_MODE, WINDOW_ROLLING))

    if window_mode == WINDOW_ROLLING:
        if (
...
DATA_SCHEMA_OPTIONS = vol.Schema(
    {
        ...
        vol.Optional(CONF_STATE_CHARACTERISTIC): SelectSelector(
            SelectSelectorConfig(
                options=list(set(list(STATS_BINARY_SUPPORT) + list(STATS_NUMERIC_SUPPORT))),
                translation_key=CONF_STATE_CHARACTERISTIC,
                mode=SelectSelectorMode.DROPDOWN,
                read_only=True,
            )
        ),
        vol.Required(WINDOW_MODE, default=WINDOW_ROLLING): SelectSelector(
            SelectSelectorConfig(
                options=SUPPORTED_WINDOW_MODES,
                mode=SelectSelectorMode.DROPDOWN,
...
class StatisticsCalendarConfigFlowHandler(SchemaConfigFlowHandler, domain=DOMAIN):
    """Handle a config flow for Statistics Calendar."""
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "statistics_calendar/start_preview",
...
async def ws_start_preview(
    ...
    preview_entity = StatisticsCalendarSensor(
        hass=hass,
        source_entity_id=entity_id,
        name=name,
        unique_id=None,
        state_characteristic=state_characteristic,
        window_mode=msg["user_input"].get(WINDOW_MODE, WINDOW_ROLLING),
        samples_max_buffer_size=sampling_size,
        samples_max_age=max_age,
        samples_keep_last=msg["user_input"].get(CONF_KEEP_LAST_SAMPLE, False),
        precision=int(msg["user_input"].get(CONF_PRECISION, DEFAULT_PRECISION)),
        percentile=int(msg["user_input"].get(CONF_PERCENTILE, 50)),
    )
